...
from networks.value_based import qmixer_ice
import torch
import numpy as np
import random
...
class EQMix:
    def __init__(self, training, agent_nb, obs_shape, states_shape, action_n, lr, gamma=0.99, batch_size=16,
        ...
        self.qmixer = qmixer_ice.EQMixer(agent_nb, states_shape, mixing_embed_dim=32).to(device)
        self.target_qmixer = qmixer_ice.EQMixer(agent_nb, states_shape, mixing_embed_dim=32).to(device)
        ...
        if self.training and self.episode_batch.size() > self.batch_size:
            for _ in range(2):
                ...
                for t in range(episode_len):
                    obs = obs_batch[:, t]#(16,agent_n,obs_size)
                    obs = np.concatenate(obs, axis=0)#(16*agent_n,obs_size)
                    obs = torch.FloatTensor(obs).to(device)
                    agent_actions, self.hidden_states = self.agents(obs, self.hidden_states)
                    agent_actions = agent_actions.view(self.batch_size, self.agent_nb, -1)#(16,agent_n,action_n)
                    mac_out.append(agent_actions)
                mac_out = torch.stack(mac_out, dim=1)#(16,episode_len,agent_n,action_n)

                chosen_action_qvals = torch.gather(mac_out[:, :-1], dim=3, index=a_batch).squeeze(3)#(16,episode_len,agent_n,)根据对应维度和index提取出对应数据
