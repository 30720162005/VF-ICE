...
from networks.value_based import vdner
import torch
import numpy as np
import random
...
class EpsilonGreedy:
    ...
    def act(self, value_action, avail_actions):
        ...
        if np.random.random() > self.epsilon:
            action = value_action.max(dim=1)[1].cpu().detach().numpy() #查找每行的最大值并返回列下标 dim=0，每列最大值；dim=1，每行最大值；不备注，所有最大值
        else:
...
class VDN:
    def __init__(self, training, agent_nb, obs_shape, states_shape, action_n, lr,gamma=0.99, batch_size=16, replay_buffer_size=5000, update_target_network=200, final_step=50000): #32
        ...
        self.vdner = vdner.VDNMixer(agent_nb, states_shape, mixing_embed_dim=32).to(device)
        self.target_vdner = vdner.VDNMixer(agent_nb, states_shape, mixing_embed_dim=32).to(device)
        
        self.target_agents.update(self.agents)
        self.target_vdner.update(self.vdner)
        
        self.params = list(self.agents.parameters())
        self.params += self.vdner.parameters()
    ...
    def save_model(self,episode,global_step, filename: object) -> object:
        all_state={'agents':self.agents.state_dict(),'optimizer':self.optimizer.state_dict(),'episodes':episode,'global_step':global_step}
    ...
    def train(self):
        ...
        if self.training and self.episode_batch.size() > self.batch_size:
            for _ in range(2):
                ...
                chosen_action_qvals = self.vdner(chosen_action_qvals)
                target_max_qvals = self.target_vdner(target_max_qvals)
                ...
                self.loss = (masked_td_error ** 2).sum() / mask.sum()

                print('loss:', self.loss)
                self.optimizer.zero_grad() #梯度归零
                self.loss.backward() #反向传播计算得到每个参数的梯度
