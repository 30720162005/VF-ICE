...
from networks.value_based import iqler
import torch
import numpy as np
import random
...
class IQL:
    def __init__(self, training, agent_nb, obs_shape, states_shape, action_n, lr, gamma=0.99, batch_size=16, replay_buffer_size=5000, update_target_network=200, final_step=50000): #32
        ...
        self.iqler = iqler.IQLer(agent_nb, states_shape, mixing_embed_dim=32).to(device)
        self.target_iqler = iqler.IQLer(agent_nb, states_shape, mixing_embed_dim=32).to(device)
        
        self.target_agents.update(self.agents)
        self.target_iqler.update(self.iqler)
        
        self.params = list(self.agents.parameters())
        self.params += self.iqler.parameters()
