...
class PCStarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
            ...
            obs_all_health=True,  # 观察值中包括可视范围内智能体的健康值
            obs_own_health=True,
            obs_last_action=False,  # 不包括上一个动作
            obs_pathing_grid=False,  # 不包括路径奖励
            obs_terrain_height=False,  # 不包括高度
            obs_instead_of_state=True,  # 不使用联合观察空间作为状态
            obs_timestep_number=False,
            state_last_action=True,
            state_timestep_number=False,
            reward_sparse=False,
            reward_only_positive=True,  # 奖励只为正
        ...
        """修改"""
        self.c0 = [np.float(1 / 3) for _ in range(self.n_agents)]
        self.c1 = [np.float(1 / 3) for _ in range(self.n_agents)]
        self.c2 = [np.float(1 / 3) for _ in range(self.n_agents)]
        self.enemy_locations = None
        self.ally_locations = None
        self.r0 = [0 for _ in range(self.n_agents)]
        self.r1 = [0 for _ in range(self.n_agents)]
        self.r2 = [0 for _ in range(self.n_agents)]

        '''写入性格特征数据'''
        # book = xlrd.open_workbook('E:\charac_data\map_3s_vs_5z_r_False.xls', formatting_info=True)
        # sheet0 = book.sheet_by_index(0)
        # sheet1 = book.sheet_by_index(1)
        # sheet2 = book.sheet_by_index(2)
        # self.rows_old0 = sheet0.nrows
        # self.rows_old1 = sheet1.nrows
        # self.rows_old2 = sheet2.nrows
        # self.book = copy(book)
        # self.sheet0 = self.book.get_sheet(0)
        # self.sheet1 = self.book.get_sheet(1)
        # self.sheet2 = self.book.get_sheet(2)
        self.c_lr = 0.1
    ...
    def step(self, actions):
        ...
        terminated = False
        """"性格特征奖励"""
        reward_c = self.get_charac_reward(actions)
        # 更新参数
        for k in range(self.n_agents):
            self.update_charact(k)
        reward = self.reward_battle() + reward_c  # 获取每一次迭代的奖励
            ...
            if not self.death_tracker_ally[al_id]:
                ...
                if al_unit.health == 0:
                    ...
                    if not self.reward_only_positive:
                        delta_deaths -= self.reward_death_value * neg_scale  # 10*0.5
    ...
    def get_obs_agent(self, agent_id):
        ...
        self.enemy_locations = np.zeros(self.n_enemies * 2, dtype=np.float)
        self.ally_locations = np.zeros(self.n_agents * 2 - 2, dtype=np.float)
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            # Enemy features
            # 每个敌人占用5位，0.是否被攻击，1.距离，2.X值，3.Y值，4.健康值
            count = 0
            for e_id, e_unit in self.enemies.items():
                e_x = e_unit.pos.x
                e_y = e_unit.pos.y
                ...
                if (
                    ...
                    self.enemy_locations[count] = enemy_feats[e_id, 2]
                    self.enemy_locations[count + 1] = enemy_feats[e_id, 2]
                    count += 2
                    ind = 4
                    if self.obs_all_health:
            ...
            # Ally features
            # 占6位，0.是否在视野范围，1.距离，2.X值，3.Y值，4.健康值，5.护盾值
            al_ids = [
            ...
            for i, al_id in enumerate(al_ids):
                ...
                if (
                    ...
                    self.ally_locations[count] = ally_feats[i, 2]
                    self.ally_locations[count + 1] = ally_feats[i, 3]
                    count += 2
    ...
    def single_Character_reward(self, c_id, action_id, o_x, o_y):
        x, y = self.action_init(action_id)
        temp = x * o_y + y * o_x
        if c_id == 1:
            reward = temp / (math.sqrt(o_x ** 2 + o_y ** 2) + 0.00001)
            reward = reward * (-1)
        else:
            reward = temp / (math.sqrt(o_x ** 2 + o_y ** 2) + 0.00001)
        return np.float(reward)

    def total_character_reward(self, agent_id, action_id, e_x, e_y, al_x, al_y):
        action_id = np.float(action_id)
        self.r0[agent_id] = 2 * self.single_Character_reward(0, action_id, e_x, e_y)
        self.r1[agent_id] = 2 * self.single_Character_reward(1, action_id, e_x, e_y)
        self.r2[agent_id] = 2 * self.single_Character_reward(2, action_id, al_x, al_y)
        r_tot = self.c0[agent_id] * self.r0[agent_id] + self.c1[agent_id] * self.r1[agent_id] + self.c2[agent_id] * \
                self.r2[agent_id]
        return r_tot

    def get_charac_reward(self, actions):
        """
        一次迭代下的性格特征奖励
        """
        reward_c = 0.0
        for k in range(self.n_agents):
            ...
            for i in range(self.n_enemies):
                e_x += np.float(self.enemy_locations[i * 2])
                e_y += np.float(self.enemy_locations[i * 2 + 1])
            for i in range(self.n_agents - 1):
                al_x += np.float(self.ally_locations[i * 2])
                al_y += np.float(self.ally_locations[i * 2 + 1])
            # 性格特征奖励
            reward_c += self.total_character_reward(k, actions[k], e_x, e_y, al_x, al_y)
        return reward_c

    def update_charact(self, agent_id):
        flag = self.r0[agent_id] + self.r1[agent_id] + self.r2[agent_id]
        if flag:
            c0 = math.exp(self.r0[agent_id])
            c1 = math.exp(self.r1[agent_id])
            c2 = math.exp(self.r2[agent_id])
            sum = c0 + c1 + c2 + 0.000001
            temp0 = self.c0[agent_id] + self.c_lr * c0 / sum
            temp1 = self.c1[agent_id] + self.c_lr * c1 / sum
            temp2 = self.c2[agent_id] + self.c_lr * c2 / sum
            temp_sum = temp0 + temp1 + temp2
            self.c0[agent_id] = temp0 / temp_sum
            self.c1[agent_id] = temp1 / temp_sum
            self.c2[agent_id] = temp2 / temp_sum

    def data_write(self, episode):
        for i in range(self.n_agents):
            self.sheet0.write(episode + self.rows_old0, i, self.c0[i])
        for i in range(self.n_agents):
            self.sheet1.write(episode + self.rows_old0, i, self.c1[i])
        for i in range(self.n_agents):
            self.sheet2.write(episode + self.rows_old0, i, self.c2[i])
        self.book.save(filename_or_stream='E:\charac_data\map_3s_vs_5z_r_False.xls')
